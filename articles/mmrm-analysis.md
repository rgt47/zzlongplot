...
The table names its rows at the left edge and hides the legend by
default, on the assumption that those names identify the groups. Here
the group names are dropped and the legend kept instead, which reads
better when the significance annotations already occupy the space above
the panel.
