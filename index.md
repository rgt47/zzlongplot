...
`zzlongplot` is an R package for configurable, dynamic visualization of
observed and change values in longitudinal datasets. Designed for both
continuous and categorical variables, `zzlongplot` supports the creation
of grouped and faceted plots, offering customization options for axes,
titles, and error representations.

This package is particularly useful for analyzing longitudinal clinical
trial data, repeated measures, or any data with a time- or
visit-dependent structure.
...
- **Dynamic Plotting**: Automatically adapts plots for continuous or
  categorical x-axis variables.
- **Observed and Change Plots**: Visualize both observed values and
  their changes relative to a baseline (`baseline_value`).
- **Grouping and Faceting**: Support for multiple grouping variables and
  faceting for stratified visualizations.
- **Custom Error Representation**: Choose between error bars or ribbons
  to represent uncertainty.
- **Combining Plots**: Use the **patchwork** package to display observed
  and change plots side-by-side.
- **Formula Interface**: Integration with formulas for specifying
  variables (`y ~ x | group`).

### **Clinical Trials Support**

- **CDISC Compliance**: Automatic recognition of standard CDISC variable
  names (AVAL, AVISITN, TRT01P, etc.)
- **Clinical Themes**: FDA and regulatory-ready plot styling with
  professional themes
- **Treatment Styling**: Predefined color schemes for placebo vs. active
  treatment visualization
- **Clinical Statistics**: 95% confidence intervals, sample size
  annotations, missing data handling
- **Regulatory Output**: Export plots in formats suitable for regulatory
  submissions
...
The following examples demonstrate basic usage of `zzlongplot`:
...
The main function for generating plots. Combines the functionality of
helper functions to produce observed plots, change plots, or both.

- **Arguments**:
  - `df`: The data frame containing the data.
  - `form`: A formula specifying the dependent (`y`) and independent
    (`x`) variables, as well as grouping variables.
...
- **`compute_stats`**: Computes summary statistics for observed and
  change values, accounting for grouping, faceting, and baseline
  (`baseline_value`).

- **`generate_plot`**: Creates a ggplot object with dynamic axis
  scaling, grouping, faceting, and error representation.

- **`parse_formula`**: Parses the formula to extract dependent,
  independent, grouping, and faceting variables.

### **Clinical Utilities**

- **[`suggest_clinical_vars()`](https://rgt47.github.io/zzlongplot/reference/suggest_clinical_vars.md)**:
  Auto-detect likely CDISC variables in a given dataset and suggest
  proper formula syntax.

- **[`validate_cdisc_data()`](https://rgt47.github.io/zzlongplot/reference/validate_cdisc_data.md)**:
  Score a dataset for CDISC compliance and report issues and
  recommendations.

- **[`get_publication_theme()`](https://rgt47.github.io/zzlongplot/reference/get_publication_theme.md)**
  and the `theme_*()` family: Regulatory- and journal-ready ggplot2
  themes
...
- **[`clinical_colors()`](https://rgt47.github.io/zzlongplot/reference/clinical_colors.md)**:
  Predefined color palettes for treatment groups following clinical
  standards.
...
Error display may be customized as follows: - `"bar"`: Error bars -
`"band"`: Error ribbons

### Faceting

Faceting allows stratified visualizations. Use the `facet_form` argument
to specify row and column facets.

### Combining Plots

Use the `"both"` option for `plot_type` to display observed and change
plots side-by-side using the **patchwork** package.
...
The `zzlongplot` package imports the following R packages: - **dplyr**:
For data manipulation. - **ggplot2**: For visualization. -
**patchwork**: For combining plots. - **RColorBrewer**: For
...
Contributions to `zzlongplot` are welcome. To contribute: 1. Fork the
repository. 2. Create a feature branch. 3. Submit a pull request.

For bug reports or feature requests, please open an issue on GitHub.

------------------------------------------------------------------------

## **PDF Rendering and Font Issues**

### **Font Requirements for PDF Output**

When using publication themes with PDF output (e.g., via
[`ggsave()`](https://ggplot2.tidyverse.org/reference/ggsave.html) or R
Markdown), font errors such as the following may occur:

    font family 'Arial' not found in PostScript font database

This happens because R’s graphics devices need explicit font
registration for PDF rendering.
...
- **Linux**: `/usr/share/fonts/truetype/liberation/` (or install
  `ttf-mscorefonts-installer`)

### **HTML Output**

HTML output (via R Markdown) works without additional setup - fonts are
...
`zzlongplot` is licensed under the GPL-3 License. See the
[LICENSE](https://rgt47.github.io/zzlongplot/LICENSE) file for details.

------------------------------------------------------------------------

## **Acknowledgments**

The `zzlongplot` package was inspired by the need for configurable tools
to visualize longitudinal data in clinical and biomedical research.

------------------------------------------------------------------------

Questions and issues may be directed to the [GitHub
Issues](https://github.com/rgt47/zzlongplot/issues) page.
