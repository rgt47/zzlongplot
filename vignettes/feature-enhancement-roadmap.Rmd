...
knitr::opts_chunk$set(
  ...
  warning = FALSE,
  message = FALSE,
  eval = TRUE  # Enable evaluation for demonstration plots
)
...
# Set up for demonstration plots with actual data
...
```{r forestplot_example, eval=TRUE}
library(forestplot)
...
```{r performance_survival, eval=FALSE}
...
```{r performance_forest, eval=FALSE}
...
```{r performance_large_data, eval=FALSE}
...
```{r architecture_diagram, eval=FALSE, echo=FALSE}
# Architecture flowchart (would be implemented as actual diagram)
# 
# Input Data (CDISC/ADaM)
#       ↓
# Formula Parser
#   ├── parse_formula() → Main plotting variables
#   ├── parse_facet_formula() → Faceting specifications  
#   └── parse_survival_formula() → Time-to-event components
#       ↓
# Data Processing Pipeline
#   ├── compute_stats() → Summary statistics
#   ├── compute_survival_stats() → KM estimates, risk tables
#   ├── compute_forest_stats() → Effect sizes, CIs
#   └── handle_missing_data() → Imputation, sensitivity
#       ↓
# Plot Generation Engine
#   ├── generate_plot() → Base ggplot construction
#   ├── add_statistical_overlays() → Tests, annotations
#   ├── apply_clinical_theme() → Regulatory styling
#   └── add_export_metadata() → Title, footnotes, metadata
#       ↓
# Output Rendering
#   ├── Static: PNG, PDF, TIFF (publication)
#   ├── Interactive: HTML widgets, Shiny apps
#   └── Regulatory: RTF, eCTD-compliant formats
```

### Memory Management and Performance Optimization

```{r performance_optimization, eval=FALSE}
...
```{r statistical_optimization, eval=FALSE}
...
```{r platform_performance, eval=FALSE}
...
```{r testing_framework, eval=FALSE}
...
```{r regulatory_validation, eval=FALSE}
...
This section provides actual working examples of the types of plots that would be enhanced in `zzlongplot`, using available R packages.

## Demonstration: Waterfall Plot for Tumor Response

```{r demo_waterfall, eval=TRUE, fig.height=4, fig.width=7}
...
ggplot(tumor_data, aes(x = patient_order, y = percent_change)) +
  ...
  facet_wrap(~ treatment_arm, scales = "free_x") +
  theme_minimal() +
  theme(
  ...
  labs(
    ...
    subtitle = "Dashed lines indicate RECIST response thresholds (-30%, +20%)",
    caption = "PR = Partial Response, SD = Stable Disease, PD = Progressive Disease"
  )
```

## Demonstration: Patient Swimmer Plot

```{r demo_swimmer, eval=TRUE, fig.height=5, fig.width=7}
...
```{r demo_forest, eval=TRUE, fig.height=5, fig.width=7}
...
```{r demo_longitudinal, eval=TRUE, fig.height=4, fig.width=7}
...
```{r demo_survival, eval=TRUE, fig.height=5, fig.width=7}
...
These demonstration plots illustrate the types of visualizations that the enhanced `zzlongplot` package would provide through its unified formula-based syntax, while maintaining the statistical rigor and regulatory compliance required for clinical trial reporting.
