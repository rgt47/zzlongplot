...
knitr::opts_chunk$set(
  ...
  warning = FALSE,
  message = FALSE,
  eval = FALSE  # Set to FALSE since many packages may not be installed
)

# Load available packages - suppress errors for demonstration purposes
suppressMessages({
  if (require(ggplot2, quietly = TRUE)) library(ggplot2)
  if (require(dplyr, quietly = TRUE)) library(dplyr)
  if (require(survival, quietly = TRUE)) library(survival)
...
check_pkg <- function(pkg) {
  if (!requireNamespace(pkg, quietly = TRUE)) {
    message(paste("Package", pkg, "not available - code examples shown for illustration"))
    return(FALSE)
  }
  return(TRUE)
}
