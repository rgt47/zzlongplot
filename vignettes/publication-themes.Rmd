...
**JAMA Theme Features:**
- Conservative color palette with dark blue-gray (#374E55) and orange (#DF8F44)
...
```{r ribbon-custom, fig.width=16, fig.height=8}
...
- **`ribbon_alpha`**: Controls transparency (0 = fully transparent, 1 = fully opaque)
...
Colour alone is a fragile channel. A figure printed in
greyscale, photocopied, or read by someone with a colour
vision deficiency needs a second cue, and the usual second
cues are line type and point shape.

`bw_print` maps both to the grouping variable in addition to
colour. It defaults to `TRUE` under `theme = "bw"` and
`FALSE` otherwise, which is what the `"bw"` theme is for, but
it is an argument in its own right and can be set either way
under any theme. The two decisions are unrelated: which house
style a figure is typeset in, and whether it survives losing
its colour.

```{r bw-print, fig.width=12, fig.height=5}
...
Before version 0.4.0 this was reachable only through
`theme = "bw"`, so selecting a journal theme silently dropped
the redundant encoding. If a figure has to be legible in
monochrome, set `bw_print = TRUE` explicitly rather than
relying on the theme to imply it.

## Matching a Document's Type Size

A figure typeset into a manuscript should carry that
document's type size, not the theme's default. `base_size`
passes a base size through to whichever theme is in effect.

```{r base-size, fig.width=12, fig.height=5}
...
Prefer this to adding a complete theme afterwards. Every
element size in these themes is defined relative to the base
size, so `base_size` scales the whole figure coherently,
whereas appending `theme_bw(base_size = 8)` discards the
journal styling along with the margin and legend settings
...
- Set `bw_print = TRUE` so line type and point shape carry the group
  as well as colour, under whichever theme the figure uses
